...
";
            ...
            var soc = sos.ToSpriterObjectCollection();
...
";
            ...
            var so2 = sos.ToRuntime(1);

            Assert.AreEqual(10f, so.Animations.First().Value.KeyFrames.First().Values.First().Value.RelativePosition.X);
            Assert.AreEqual(10f, so2.Animations.First().Value.KeyFrames.First().Values.First().Value.RelativePosition.X);

            Assert.AreEqual(-10f, so.Animations.First().Value.KeyFrames.First().Values.First().Value.RelativePosition.Y);
            Assert.AreEqual(10f, so2.Animations.First().Value.KeyFrames.First().Values.First().Value.RelativePosition.Y);

            Assert.AreEqual(1f, so.Animations.First().Value.TotalTime);
            Assert.AreEqual(1f, so2.Animations.First().Value.TotalTime);
        }
    }
}
